...
use nix::unistd::{Pid, geteuid, getpid};
#[cfg(target_os = "linux")]
use std::fs;
...
///
/// Now uses the same root as user config (single-root approach)
pub fn platform_system_config_dir() -> PathBuf {
    KodegenConfig::config_dir()
        .unwrap_or_else(|_| PathBuf::from(".config/kodegen/config"))
}

/// User configuration directory
///
/// Returns the root kodegen directory (single-root approach)
pub fn platform_user_config_dir() -> PathBuf {
    KodegenConfig::user_config_dir()
        .unwrap_or_else(|_| PathBuf::from(".config/kodegen"))
}

/// Runtime directory for PID files and sockets
///
/// Uses single-root approach: always returns {root}/state/
/// No privilege-dependent logic - all data in user space
pub fn platform_runtime_dir(_is_elevated: bool) -> PathBuf {
    KodegenConfig::state_dir()
        .unwrap_or_else(|_| PathBuf::from(".config/kodegen/state"))
}

/// Log directory
///
/// Uses single-root approach: always returns {root}/logs/
/// No privilege-dependent logic - all data in user space
pub fn platform_log_dir(_is_elevated: bool) -> PathBuf {
    KodegenConfig::log_dir()
        .unwrap_or_else(|_| PathBuf::from(".config/kodegen/logs"))
}
